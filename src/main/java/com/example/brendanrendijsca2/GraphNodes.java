...
//    public List<GraphLinks> adjList = new ArrayList<>();
    ...
    public static <T> List<List<GraphNodes<Station>>> FindAllPathsDepthFirst(GraphNodes<Station> from, List<GraphNodes<Station>> encountered, T lookingfor){
        List<List<GraphNodes<Station>>> result=null, temp2;
        if(from.station.equals(lookingfor)) { //Found it
            List<GraphNodes<Station>> temp=new ArrayList<>(); //Create new single solution path list
        ...
        for(GraphNodes<Station> adjNode : from.neighbours){
...
//for recursive call!
                if(temp2!=null) { //Result of the recursive call contains one or more paths to the solution node
                    for(List<GraphNodes<Station>> x : temp2) //For each partial path list returned
